...
// underscore did.
export const FORMAT_MARK = { bold: '**', italic: '*', underline: '__', strike: '~~' };
...
function insertSpacing(text, start, end) {
    const before = text.slice(0, start);
    const after = text.slice(end);
    const next = `${before}\n\n${after}`;
    const caret = start + 2;
...
function toggleLinePrefix(text, at, prefix, exclusiveWith = []) {
...
function paragraphScope(lines, selStart, selEnd) {
    const startsAt = [];
    let off = 0;
    for (const l of lines) {
        startsAt.push(off);
    ...
    const lineOf = (pos) => {
        let i = 0;
        while (i + 1 < lines.length && startsAt[i + 1] <= pos)
            i++;
        return i;
    };
    const hasInk = (i) => lines[i].trim().length > 0;
    ...
    if (selEnd > selStart && last > first && selEnd === startsAt[last])
        last--;
    ...
    const paraOf = (line) => paras.find(p => line >= p.startLine && line <= p.endLine);
    const pFirst = hasInk(first) ? paraOf(first) : undefined;
    if (pFirst)
        first = pFirst.startLine;
...
function outdentParagraphs(text, selStart, selEnd) {
    const lines = text.split('\n');
    ...
    const nextLines = lines.map((l, i) => {
        if (affected.has(i) && l.startsWith(TAB)) {
            removed.add(i);
...
function indentParagraphs(text, selStart, selEnd) {
    const lines = text.split('\n');
    const { affected, lineOf } = paragraphScope(lines, selStart, selEnd);
    const TAB = LINE_DIRECTIVE.indent;
...
const ALIGN_PREFIXES = [LINE_DIRECTIVE['align-center'], LINE_DIRECTIVE['align-right']];
...
export function marksAt(text, caret) {
    const within = (mark) => {
        let i = 0;
        while (i < text.length) {
            ...
            if (close === -1)
                return false; // unpaired: not a run, so nothing is inside it
            if (caret >= open && caret <= close + mark.length)
                return true;
            i = close + mark.length;
        }
        return false;
...
export function applyFormat(text, selStart, selEnd, action) {
    const start = Math.min(selStart, selEnd);
    const end = Math.max(selStart, selEnd);
    if (action === 'bold')
        return wrapSelection(text, start, end, FORMAT_MARK.bold);
    if (action === 'italic')
        return wrapSelection(text, start, end, FORMAT_MARK.italic);
    if (action === 'underline')
        return wrapSelection(text, start, end, FORMAT_MARK.underline);
    if (action === 'strike')
        return wrapSelection(text, start, end, FORMAT_MARK.strike);
    if (action === 'heading')
        return cycleHeading(text, start);
    if (action === 'bullet')
        return toggleLinePrefix(text, start, LINE_DIRECTIVE.bullet);
    if (action === 'quote')
        return toggleLinePrefix(text, start, LINE_DIRECTIVE.quote);
    ...
    if (action === 'indent')
        return indentParagraphs(text, start, end);
    if (action === 'outdent')
        return outdentParagraphs(text, start, end);
    if (action === 'align-center')
        return toggleLinePrefix(text, start, LINE_DIRECTIVE['align-center'], ALIGN_PREFIXES);
    if (action === 'align-right')
        return toggleLinePrefix(text, start, LINE_DIRECTIVE['align-right'], ALIGN_PREFIXES);
...
export function visibleText(raw) {
    ...
    const cut = (idx, ranges) => {
        if (ranges.length === 0)
            return idx;
        const out = [];
        let r = 0;
        for (let i = 0; i < idx.length; i++) {
            while (r < ranges.length && i >= ranges[r][1])
                r++;
    ...
    // Newlines are never removed, so line boundaries are stable within a pass.
    {
        ...
        for (;;) {
            ...
            if (take > 0)
                ranges.push([lineStart, lineStart + take]);
            if (nl < 0)
                break;
            lineStart = nl + 1;
        }
 ...
 */
export function toRawRange(v, start, end) {
    if (end <= start) {
...
export function stripMarkdownConventions(text) {
...
export function stripMarkdownConventionsReference(text) {
