...
if (process.env.HARNESS_PARKED === '1') {
  ...
  await withHarness(async (app) => {
    ...
    await app.evalJs(`(() => {
      const now = new Date().toISOString();
      window.wrizoCreateJournalPage({ id: 'ab2-script-parked-typewriter', text: '', pageType: 'script', script: { v: 1, scenes: [{ id: 's1', heading: { id: 's1', t: 'scene', text: '' }, body: [] }] }, createdAt: now, source: null, origin: null });
    })()`);
    await app.reload();
