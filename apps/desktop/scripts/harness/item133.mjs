...
await withHarness(async (app) => {
  ...
  // wrong reason.
  await app.evalJs("(() => { const __t = document.querySelector('.crumb-rename'); if (!__t) throw new Error(\"no dispatchEvent target: document.querySelector('.crumb-rename')\"); return __t.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true })); })()");
  await sleep(250);
  ...
  await app.evalJs("(() => { const __t = document.querySelector('.wz-strip-item[data-category=page]'); if (!__t) throw new Error(\"no click target: document.querySelector('.wz-strip-item[data-category=page]')\"); return __t.click(); })()");
  await sleep(600);
