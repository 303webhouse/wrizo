...
const freshScriptPage = async (app) => {
  ...
  await app.evalJs(`(() => {
    ...
    window.wrizoCreateJournalPage({ id: 'cd1-script', text: '', pageType: 'script', script: { v: 1, scenes: [{ id: headingId, heading: { id: headingId, t: 'scene', text: '' }, body: [] }] }, createdAt: now, source: null, origin: null });
  })()`);
  await app.reload();
...
await withHarness(async (app) => {
  ...
  await openSliver(app);
  await sleep(200);
  await app.evalJs("(() => { const __t = document.querySelector('.wz-sliver-goal-edit'); if (!__t) throw new Error(\"no click target: document.querySelector('.wz-sliver-goal-edit')\"); return __t.click(); })()");
  await sleep(100);
  await app.evalJs("(() => { const __t = document.querySelector('.wz-sliver-goal-edit-clear'); if (!__t) throw new Error(\"no click target: document.querySelector('.wz-sliver-goal-edit-clear')\"); return __t.click(); })()");
  await sleep(200);
