...
await withHarness(async (app) => {
  ...
  await app.evalJs("(() => { const __t = [...document.querySelectorAll('.board-sheet button')].find(b => b.textContent.trim() === 'Cancel'); if (!__t) throw new Error(\"no click target: [...document.querySelectorAll('.board-sheet button')].find(b => b.textContent.trim() === 'Cancel')\"); return __t.click(); })()");
  await sleep(150);

  await app.evalJs("(() => { const __t = document.querySelector('.wz-pageface-verb-port'); if (!__t) throw new Error(\"no click target: document.querySelector('.wz-pageface-verb-port')\"); return __t.click(); })()");
  ...
  await sleep(150);
  await app.evalJs("(() => { const __t = document.querySelector('.dz-rename'); if (!__t) throw new Error(\"no blur target: document.querySelector('.dz-rename')\"); return __t.blur(); })()");
  await sleep(150);
  const drawerGroupPresent = await app.evalJs("!!document.querySelector('.dz-group')");
  if (drawerGroupPresent) {
    await app.evalJs("(() => { const __t = [...document.querySelectorAll('.dz-more')].find(b => b.textContent.includes('Create New')); if (!__t) throw new Error(\"no click target: [...document.querySelectorAll('.dz-more')].find(b => b.textContent.includes('Create New'))\"); return __t.click(); })()");
    await sleep(100);
    ...
    await app.evalJs("(() => { const __t = [...document.querySelectorAll('.dz-createnew .dz-more')].find(b => b.textContent === 'New Binder'); if (!__t) throw new Error(\"no click target: [...document.querySelectorAll('.dz-createnew .dz-more')].find(b => b.textContent === 'New Binder')\"); return __t.click(); })()");
