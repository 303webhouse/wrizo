...
const freshBoard = async (app, boardId, boxes, width = 1400, height = 900) => {
  ...
  await app.evalJs(`(() => {
    ...
    window.wrizoCreateJournalPage({ id: ${JSON.stringify(boardId)}, text: 'BG2 Board', pageType: 'board', boxes: ${JSON.stringify(boxes)}, createdAt: now, origin: null });
  })()`);
  await app.reload();
...
const freshPage = async (app, pageId, text = '', width = 1400, height = 900) => {
  ...
  await app.evalJs(`(() => {
    const now = new Date().toISOString();
    window.wrizoCreateJournalPage({ id: ${JSON.stringify(pageId)}, text: ${JSON.stringify(text)}, projectId: null, origin: 'loose', createdAt: now });
  })()`);
  await app.reload();
