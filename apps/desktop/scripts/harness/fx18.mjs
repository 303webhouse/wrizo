...
await withHarness(async (app) => {
  // ── S1 — the arrow mirrors (on a framed prose page, both grips, both states) ──
  {
    ...
    await app.evalJs("(() => { const __t = document.querySelector('.wz-sliver-grip'); if (!__t) throw new Error(\"no click target: document.querySelector('.wz-sliver-grip')\"); return __t.click(); })()"); await sleep(250);
    await app.evalJs("(() => { const __t = document.querySelector('.wz-tutor-grip'); if (!__t) throw new Error(\"no click target: document.querySelector('.wz-tutor-grip')\"); return __t.click(); })()"); await sleep(250);
  ...
  for (const s of writingSurfaces) {
    for (const [w, h] of writingLegs) {
      ...
      await app.evalJs("(() => { const __t = document.querySelector('.wz-sliver-grip'); if (!__t) throw new Error(\"no click target: document.querySelector('.wz-sliver-grip')\"); return __t.click(); })()"); await sleep(200);
      await app.evalJs("(() => { const __t = document.querySelector('.wz-tutor-grip'); if (!__t) throw new Error(\"no click target: document.querySelector('.wz-tutor-grip')\"); return __t.click(); })()"); await sleep(300);
