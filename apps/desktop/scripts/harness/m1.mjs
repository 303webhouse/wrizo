...
await withHarness(async (app) => {
  ...
  const planId = await app.evalJs(`(() => {
    ...
    // ITEM 85-C — three raw writes become three seam calls, and the plan's id is
    // now GENERATED (see the PARKED assertion at the bottom of this file).
    //   · createStoryPlan derives beatNotes from beatIds AND stamps
    //     project.storyPlanId, which the first two raw writes did separately.
    //   · setCurrentBeat then moves the current beat to beat[1]. createStoryPlan
    //     always makes the FIRST beat current, and a plan whose current beat is
    //     not the opening one is this fixture's entire subject.
    //   · wrizoPatchEntry stamps the page's beatId; it returns null on a missing
    //     row, which is the same no-op the old if-ei-check provided.
    // The ERR-no-project guard is kept — reading storage is lawful.
    const projects = JSON.parse(localStorage.getItem('writer-studio-projects') || '[]');
    if (!projects.some(p => p.id === projectId)) return 'ERR-no-project';

    const plan = window.wrizoCreateStoryPlan(projectId, 'story_circle', beatIds);
    window.wrizoSetCurrentBeat(plan.id, beatIds[1]);
    window.wrizoPatchEntry(pageId, { beatId: beatIds[1] });

    return plan.id;
  })()`);
  // ITEM 85-C LIVE SUCCESSOR — the original asserted the fixture's own id
  // convention (startsWith 'm1-plan-'), which the seam cannot satisfy because
  // createStoryPlan generates the id. The CLAIM is unchanged — a plan was seeded
  // and the page is attached to beat[1] — so it is re-asserted against the plan's
  // existence and its actual current beat, which is the thing that was really
  // being checked. The original is PARKED verbatim at the bottom of this file.
  ok('fixture 1: StoryPlan (story_circle) seeded, page attached to beat[1] [ITEM 85-C successor: the plan id is generated by the store, so its identity is proven by existence + the current beat rather than by a fixture-authored prefix]',
  ...
  await app.evalJs(`(() => {
    ...
    // ITEM 85-C — createStoryPlan derives all 15 beatNotes from beatIds and
    // stamps project.storyPlanId; setCurrentBeat then moves the current beat to
    // the anchor, which createStoryPlan cannot (it always makes the first
    // current) and which is the whole point of this fixture's centering test.
    const plan = window.wrizoCreateStoryPlan(projectId, 'save_the_cat', beatIds);
    window.wrizoSetCurrentBeat(plan.id, '${anchorId}');
  })()`);
  ...
  await app.evalJs(`(() => {
    ...
    // ITEM 85-C — createStoryPlan makes every beat 'empty', so the one COMPLETE
    // beat this fixture needs is set through setBeatStatus, and the current beat
    // through setCurrentBeat. Both are the store's own paths.
    const plan = window.wrizoCreateStoryPlan(projectId, 'three_act', beatIds);
    window.wrizoSetBeatStatus(plan.id, 'midpoint', 'complete');
    window.wrizoSetCurrentBeat(plan.id, 'midpoint');
  })()`);
  ...
  const planIdB = await app.evalJs(`(() => {
    ...
    const beatIds = ${JSON.stringify(threeActBeats)};
    // ITEM 85-C — as project A above, without the completed beat: this plan's
    // beats are all empty and only its CURRENT beat is moved.
    const plan = window.wrizoCreateStoryPlan(projectId, 'three_act', beatIds);
    window.wrizoSetCurrentBeat(plan.id, 'midpoint');
    return plan.id;
  })()`);
...
if (process.env.HARNESS_PARKED === '1') {
  ...
  // === ITEM 85-C — SUPERSEDED (the fixture-authored plan id) ==================
  // ORIGINAL, verbatim:
  //
  ...
  // createStoryPlan, which GENERATES the id — a ruled constraint of item 85-C
  // (ids are returned, never supplied, because overriding them would mean
  // changing the store's creators rather than the door). So a prefix the fixture
  // no longer chooses can no longer be asserted.
  //
  // WHAT THE CHECK WAS ACTUALLY FOR, and where it lives now: that a plan was
  // seeded and the page was attached to beat[1]. The LIVE SUCCESSOR in this
  // file's fixture-1 section asserts exactly that — the plan exists (a non-empty
  // id that is not the ERR sentinel) — and the beat attachment is proven by the
  // checks that read the rendered beat, which never depended on the id's shape.
  // Nothing is lost: the prefix was a fixture convention, never a product claim.
  pok('PARKED (was "fixture 1: StoryPlan (story_circle) seeded, page attached to beat[1]" asserting planId.startsWith(\'m1-plan-\')) — ITEM 85-C: the plan id is now generated by createStoryPlan, so a fixture-authored prefix is unassertable. Live successor in the fixture-1 section asserts the plan EXISTS instead, which is what the check was for',
    true, 'superseded by item 85-C: ids are returned by the store, never supplied by the fixture');
  await withHarness(async (app) => {
    await app.goto('/');
    await app.evalJs("localStorage.clear(); localStorage.setItem('wrizo-first-run-complete', '1')");
    await app.reload();
    ...
    await app.evalJs(`(() => {
      // ITEM 85-C — createStoryPlan with NO beat ids reproduces this plan exactly:
      // beatNotes [] and currentBeatId null, both derived from the empty list. It
      // also stamps project.storyPlanId, which the first raw write did by hand.
      window.wrizoCreateStoryPlan('${projectIdParked}', 'story_circle', []);
    })()`);
    await app.reload();
