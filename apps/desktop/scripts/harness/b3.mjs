...
const freshBoard = async (app, boardId, boxes, width = 1400, height = 900) => {
  ...
  await app.evalJs(`(() => {
    ...
    // ITEM 85-C — the project's id is GENERATED by createProject, not supplied,
    // and it is used only here (to home the board), so the returned id stays
    // local and nothing downstream changes. type:'creative' and storyPlanId:null
    // are createProject's own defaults. (No backticks in comments inside an
    // evalJs template literal — they close it.)
    const project = window.wrizoCreateProject('B3 Project');
    window.wrizoCreateJournalPage({ id: ${JSON.stringify(boardId)}, text: 'B3 Board', projectId: project.id, pageType: 'board', boxes: ${JSON.stringify(boxes)}, createdAt: now, origin: null });
  })()`);
  await app.reload();
