...
function rowToJournalEntry(r: any) {
  return {
    ...
    // ITEM 144 — a board's "beside" connections (one object, one array; see
    // migrate.ts for the shape). The exact origin/script/tutor/pageSettings recipe:
    // SQL null -> JS undefined, never `null` and never an empty object, so a board
    // that has never been connected stays byte-identical to today. NOTHING here
    // resolves or reverse-reads: the mapper hands the stored object straight back —
    // the reverse scan that lets the OTHER end show the connection is the client's.
    besideLinks: r.beside_links ?? undefined,
      ...
      await pool.query(
        `insert into journal_entries
           (id, user_id, project_id, text, session_id, starred, source, shelved, beat_id, page_type,
            order_index, imported_at, boxes, script, origin, tutor, tags, routed_project_ids, strokes, deleted_at, created_at, updated_at, plan_board_id, page_settings, beside_links)
         ...
         on conflict (id) do update set
           ...
           /* ITEM 144 — a board's beside connections, riding the SAME
              last-writer-wins guard (the updated_at comparison below) as every
              column above it. A CONNECTION CHANGE IS A BOARD CHANGE, so there is
              nothing to special-case. Whole-column resolution is the known limit —
              see migrate.ts. */
           beside_links = excluded.beside_links
        ...
        [e.id, userId, e.projectId ?? null, e.text ?? '', e.sessionId ?? null,
         ...
         e.deletedAt ?? null, e.createdAt, e.updatedAt, e.planBoardId ?? null, JSON.stringify(e.pageSettings ?? null), JSON.stringify(e.besideLinks ?? null)],
      );
    } catch (err) {
